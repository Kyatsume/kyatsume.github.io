...
$.extend(CsvCalc.prototype, /** @lends CsvCalc.prototype */ {
  ...
  showAtHtml: function (data) {
    ...
    for (var m = 0; m < data.length; m++) {
      ...
      var elem_totalMobs = $(clone).find('[data-csvcalc-calculate]');
      $(elem_totalMobs).attr('data-csvcalc-calculate', $(elem_totalMobs).text());
  ...
  calcTotal2: function () {
    var monster = this;
    $(document).on('change', $(monster.elem).find('[data-csvcalc-mobsinput]'), function (ev2) {
      // バリデーションを行う
      var cMobs = monster.validateNumber.call(monster, $(ev2.target).val());
      $(ev2.target).val(cMobs); // 画面上の全角数字は、ここで半角となる。

      // 合計を算出・表示
      var parent2 = $(ev2.target).parents('[data-csvcalc-repeat]');
